...
const Sponsors = () => {
  ...
  // Org Admins always can; staff need the 'sponsors'
  // section granted under Accessibility.
  const canManage =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        'Could not save sponsor. Please try again.'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async (s) => {
    const ok = await confirm({
      ...
      message: `"${s.name}" and their sponsorship history will be permanently removed.`,
      confirmLabel: 'Delete',
      variant: 'danger'
    });
  ...
  const handleDeleteSponsorship = async (
    ...
    const ok = await confirm({
      title:
        'Delete this sponsorship record?',
      confirmLabel: 'Delete',
      variant: 'danger'
    });
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            ...
            and which projects or campaigns
            they back.
          </p>
        </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canManage ? (
                      <select
                        ...
                        onChange={(e) =>
                          handleSponsorshipStatus(
                            sp,
                            e.target.value
                          )
                        }
                        className={`text-xs font-semibold rounded-full px-2 py-1 border-0 ${statusBadgeClass(
                          sp.status
                        )}`}
                      >
                        {STATUSES.map(
