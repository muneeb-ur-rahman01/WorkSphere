...
import {
  BarChart3, Calendar, CalendarDays, Users, CheckSquare, UserPlus, Loader2, AlertTriangle
} from 'lucide-react';
import {
  ResponsiveContainer, AreaChart, Area, ComposedChart, Bar, Line,
  BarChart, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend
} from 'recharts';
...
const Analytics = () => {
  ...
  const kpiCards = [
    {
      label: 'Total Camps', value: kpis.totalCamps ?? 0, icon: Calendar,
      accent: 'from-blue-500 to-cyan-500',
      sub: `${kpis.periodCamps ?? 0} scheduled ${RANGE_CAPTION[range].toLowerCase()}`
    },
    {
      label: 'Total Events', value: kpis.totalEvents ?? 0, icon: CalendarDays,
    ...
    },
    {
      ...
      sub: `${kpis.activeUsers ?? 0} active • ${kpis.pendingUsers ?? 0} pending`
    },
    {
      label: 'New Users', value: kpis.periodNewUsers ?? 0, icon: UserPlus,
      accent: 'from-amber-500 to-orange-500',
      sub: `Joined ${RANGE_CAPTION[range].toLowerCase()}`
    },
    {
      label: 'Task Completion', value: `${kpis.taskCompletionRate ?? 0}%`, icon: CheckSquare,
      accent: 'from-emerald-500 to-green-600',
      sub: `${kpis.completedTasks ?? 0} of ${kpis.totalTasks ?? 0} tasks completed`
    }
  ];
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-indigo-100 mt-1 max-w-xl">
                  Track camps, events, personnel and task progress for your organization.
                </p>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-indigo-100/40 p-6">
              <div className="mb-4">
                <h2 className="text-xl font-bold text-black">Camps & Events Progress</h2>
                <p className="text-sm text-gray-600 mt-1">
                  Number of camps and events scheduled per period ({RANGE_CAPTION[range].toLowerCase()})
                </p>
              </div>
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={trend} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorCamps" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Tooltip />
                  <Legend />
                  <Area type="monotone" dataKey="camps" name="Camps" stroke="#0ea5e9" fill="url(#colorCamps)" strokeWidth={2} />
                  <Area type="monotone" dataKey="events" name="Events" stroke="#a855f7" fill="url(#colorEvents)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-indigo-100/40 p-6">
              <div className="mb-4">
                <h2 className="text-xl font-bold text-black">User Growth & Activity</h2>
                <p className="text-sm text-gray-600 mt-1">
                  New sign-ups per period (bars) vs. total personnel over time (line)
                </p>
              </div>
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={trend} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="newUsers" name="New Users" fill="#f59e0b" radius={[6, 6, 0, 0]} barSize={28} />
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              ...
              {/* Personnel by Role */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-indigo-100/40 p-6">
                <h2 className="text-lg font-bold text-black mb-1">Personnel by Role</h2>
                <p className="text-xs text-gray-500 mb-4">Active staff breakdown</p>
                {personnelByRole.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={personnelByRole} layout="vertical" margin={{ left: 10 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                      <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11 }} />
                      <YAxis type="category" dataKey="key" tick={{ fontSize: 11 }} width={90} />
                      <Tooltip />
                      <Bar dataKey="count" name="Staff" fill="#4f46e5" radius={[0, 6, 6, 0]} barSize={18} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="text-sm text-gray-400 py-16 text-center">No active personnel yet.</p>
                )}
              </div>

              {/* Events by Type */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-indigo-100/40 p-6">
                <h2 className="text-lg font-bold text-black mb-1">Events by Type</h2>
                <p className="text-xs text-gray-500 mb-4">All-time distribution</p>
                {eventsByType.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={eventsByType} dataKey="count" nameKey="key" innerRadius={45} outerRadius={75} paddingAngle={3}>
                  ...
                  <p className="text-sm text-gray-400 py-16 text-center">No events created yet.</p>
                )}
              </div>

              {/* Task Status Breakdown */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-indigo-100/40 p-6">
                <h2 className="text-lg font-bold text-black mb-1">Task Status</h2>
                <p className="text-xs text-gray-500 mb-4">All-time distribution</p>
                {taskStatusBreakdown.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={taskStatusBreakdown} dataKey="count" nameKey="key" innerRadius={45} outerRadius={75} paddingAngle={3}>
                        {taskStatusBreakdown.map((entry, idx) => (
                          <Cell key={entry.key} fill={PIE_COLORS[(idx + 2) % PIE_COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip />
                      <Legend wrapperStyle={{ fontSize: 11 }} />
                  ...
                  <p className="text-sm text-gray-400 py-16 text-center">No tasks created yet.</p>
                )}
              </div>
