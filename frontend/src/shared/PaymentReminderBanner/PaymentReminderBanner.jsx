...
// is true — i.e. 2 or fewer days remain in the org's 7-day free trial and
// no plan has been activated yet (computed server-side in
// backend/middleware/subscriptionAccess.js). Dismissal is session-only
// (component state, not persisted) so it reappears on the next login while
// the trial remains unconverted. This is a secondary, inline reminder — the
...
const PaymentReminderBanner = ({ subscription, planLabel, payHref = '#billing' }) => {
  ...
  const isTrial = subscription.trialStatus === 'Active';
  ...
  return (
    <div className="rounded-2xl border border-red-200 bg-red-50 p-5 shadow-sm flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-red-800">{isTrial ? 'Free Trial Ending Soon' : 'Payment Reminder'}</p>
        <p className="text-sm text-red-700 mt-1 leading-6">
          {isTrial ? (
            <>
              Your free trial has <strong>{subscription.trialDaysRemaining} day{subscription.trialDaysRemaining === 1 ? '' : 's'} left</strong>.
