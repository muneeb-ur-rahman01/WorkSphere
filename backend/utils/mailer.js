...
const sendPasswordResetEmail = async ({ to, fullName, resetUrl }) => {
  ...
  const text =
    `Hi ${fullName || 'there'},\n\n` +
    `We received a request to reset the password for your WorkSphere account (${to}).\n\n` +
  ...
  const html = `
    <div style="font-family: -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; color: #111;">
      ...
      <p>We received a request to reset the password for your WorkSphere account (<strong>${escapeHtml(to)}</strong>).</p>
